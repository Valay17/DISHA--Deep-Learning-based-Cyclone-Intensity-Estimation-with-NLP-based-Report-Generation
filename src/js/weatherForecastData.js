...
export const weatherForecastData = async (data, key, currentUnit) => { // Accept currentUnit as parameter
  ...
  if (data.lat && data.lon) {
    API_URL = `https://api.openweathermap.org/data/2.5/forecast?lat=${data.lat}&lon=${data.lon}&appid=${key}&units=${currentUnit}`;
  } else {
    API_URL = `https://api.openweathermap.org/data/2.5/forecast?q=${data}&appid=${key}&units=${currentUnit}`;
  }
